...
if ! command -v ansible >/dev/null; then
  ...
  if [[ ! -z ${YUM} ]]; then
    yum install -y git python python-devel gcc glibc glibc-common gd gd-devel
  elif [[ ! -z ${APT_GET} ]]; then
...
find "/vagrant/$PLAYBOOK_DIR" \( -name "requirements_windows.yml" -o -name "requirements_windows.txt" \) -exec sudo ansible-galaxy install -r {} \;
